...
$stmt = $conn->prepare("SELECT id, name, email, phone, address, password, avatar, token, zalo_id, facebook_id, role FROM users WHERE email = ?");
...
$stmt->bind_result($id, $name, $email_db, $phone, $address, $hashedPassword, $avatar, $token_db, $zalo_id, $facebook_id, $role);
$stmt->fetch();
